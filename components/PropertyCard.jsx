	...
	return (
		<div className='relative overflow-hidden shadow-md rounded-xl'>
			...
			<div className='p-4'>
				...
				{handleUnsave && <button onClick={(e) => { handleUnsave(property._id); setId(property._id) }} className='absolute top-[10px] left-[10px] bg-white px-4 py-2 rounded-lg text-red-700 font-semibold text-right md:text-center lg:text-right flex gap-1 items-center'>{(id == property._id) && isLoadingUnsave ? <Spinner margin='0px' size={15} color='#222a' /> : <FaBookmark />} {(id == property._id) && isLoadingUnsave ? 'Removing' : 'Unsave'}</button>}
